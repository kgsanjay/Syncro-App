...
<head>
    ...
    <title><?= \Syncro\Security\SecurityManager::sanitizeOutput($pageTitle ?? __('Guest Portal')) ?></title>
    ...
    <style>
        ...
        @media print {
            ...
            .print-border { border: 1px solid var(--header); padding: 20px; }
        }
    </style>
</head>
    ...
    <div class="bg-[var(--white)] border border-[var(--border)] rounded-b-lg shadow-sm p-8 print-border">
        ...
        <?php if (isset($_GET['success'])): ?>
            <div class="mb-6 bg-green-50 border-l-4 border-green-500 p-4 rounded no-print">
                <p class="text-sm font-bold text-green-800"><?= __('Check-in details updated successfully!') ?></p>
            </div>
        <?php endif; ?>
        ...
        <div class="hidden print:block mb-8 text-center border-b border-[var(--border)] pb-4">
            ...
            <h2 class="text-xl font-bold mt-2"><?= __('Guest Invoice') ?></h2>
        ...
        <div class="grid grid-cols-2 gap-6 mb-8">
            <div>
                <h3 class="text-xs font-bold text-[var(--header)] uppercase tracking-wider mb-1"><?= __('Guest Details') ?></h3>
            ...
            <div class="text-right">
                <h3 class="text-xs font-bold text-[var(--header)] uppercase tracking-wider mb-1"><?= __('Stay Details') ?></h3>
        ...
        <!-- Self <?= __('Check-in') ?> Form (No Print) -->
        <div class="mb-10 no-print border-t border-[var(--border)] pt-8">
            <h2 class="text-xl font-bold text-[var(--header)] mb-4"><?= __('Pre-Arrival Check-in') ?></h2>
            <form action="<?= base_url('/guest/portal/update') ?>" method="POST" enctype="multipart/form-data" class="space-y-4 bg-gray-50 p-6 rounded border border-gray-200">
                <input type="hidden" name="booking_id" value="<?= $booking['id'] ?>">
                
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-bold text-[var(--header)] mb-1"><?= __('Expected Arrival Time') ?></label>
                    ...
                    <div>
                        <label class="block text-sm font-bold text-[var(--header)] mb-1"><?= __('Upload ID Document (PDF/Image)') ?></label>
        ...
        <div class="border-t border-[var(--border)] pt-8">
            <div class="flex justify-between items-center mb-4">
                <h2 class="text-xl font-bold text-[var(--header)]"><?= __('Guest Folio & Invoice') ?></h2>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="border-b-2 border-[var(--header)]">
                        <th class="py-2 text-sm font-bold text-[var(--header)]"><?= __('Description') ?></th>
                        <th class="py-2 text-sm font-bold text-[var(--header)] text-center"><?= __('Qty') ?></th>
                        <th class="py-2 text-sm font-bold text-[var(--header)] text-right"><?= __('Amount') ?></th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-[var(--border)]">
                    ...
                    <tr>
                        <td class="py-3">
                            <div class="font-bold text-[var(--header)]"><?= __('Room Charges') ?></div>
                ...
                <tfoot>
                    <tr class="border-t-2 border-[var(--header)]">
                        <td colspan="2" class="py-4 text-right font-bold text-[var(--header)] uppercase tracking-wider">Total <?= __('Amount') ?> Due</td>
            ...
            <div class="mt-8 flex justify-end no-print">
                ...
                <?php else: ?>
                    <a href="<?= base_url() ?>/payment/checkout?booking_id=<?= $booking['id'] ?>&token=<?= htmlspecialchars($_GET['token'] ?? '') ?>" class="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-8 rounded shadow-lg transition-colors flex items-center">
