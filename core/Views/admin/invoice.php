...
<head>
    <meta charset="UTF-8">
    <title>Folio #<?= htmlspecialchars((string)$booking['id']) ?></title>
...
<div class="max-w-4xl mx-auto bg-white p-10 border border-gray-200 shadow-sm relative">
    ...
    <div class="no-print absolute top-4 right-4 flex space-x-4">
        <a href="/user/dashboard" class="px-4 py-2 bg-gray-200 text-gray-700 rounded hover:bg-gray-300 transition text-sm font-semibold">← Back</a>
    ...
    <div class="flex justify-between items-start border-b-2 border-gray-100 pb-8 mb-8">
        <div>
            <h1 class="text-3xl font-extrabold text-blue-900 tracking-tight"><?= htmlspecialchars($booking['hotel_name']) ?></h1>
            <p class="text-gray-500 mt-1"><?= nl2br(htmlspecialchars($booking['hotel_address'])) ?></p>
            <p class="text-gray-500"><?= htmlspecialchars($booking['hotel_phone']) ?> | <?= htmlspecialchars($booking['hotel_email']) ?></p>
        </div>
    ...
    <div class="grid grid-cols-2 gap-8 mb-8">
        <div>
            ...
            <p class="text-lg font-semibold text-gray-800"><?= htmlspecialchars($booking['guest_name']) ?></p>
            <p class="text-gray-600"><?= htmlspecialchars($booking['guest_email']) ?></p>
            <p class="text-gray-600"><?= htmlspecialchars($booking['guest_phone']) ?></p>
        </div>
        <div>
            ...
            <table class="w-full text-sm">
                ...
                <tr><td class="py-1 text-gray-600">Room Type:</td><td class="font-semibold text-right"><?= htmlspecialchars($booking['room_type_name']) ?></td></tr>
                <tr><td class="py-1 text-gray-600">Status:</td><td class="text-right"><span class="px-2 py-0.5 text-xs font-bold uppercase tracking-wider rounded <?= $statusBadgeColor ?>"><?= htmlspecialchars($booking['payment_status']) ?></span></td></tr>
            </table>
        </div>
    </div>
        ...
        <table class="w-full text-sm text-left">
            ...
            <tbody>
                ...
                <tr class="border-b border-gray-100">
                    ...
                    <td class="py-3 px-4 text-gray-600"><?= htmlspecialchars($p['payment_method']) ?></td>
                    <td class="py-3 px-4 text-gray-600 font-mono text-xs"><?= htmlspecialchars($p['transaction_id']) ?></td>
    ...
    <div class="mt-16 pt-8 border-t border-gray-100 text-center text-xs text-gray-400">
        <p>Thank you for choosing <?= htmlspecialchars($booking['hotel_name']) ?>.</p>
