...
class AgodaChannel implements OtaChannelInterface
{
    // Standard Agoda YCS API Endpoint
    private const ENDPOINT = 'https://api-ycs.agoda.com/xml/v15'; 
...
XML;
    ...
    public function fetchBookings(string $hotelId): array
    {
        ...
        // Standard OTA_ReadRQ to fetch unread bookings from Agoda
        $xmlPayload = <<<XML
...
XML;
    ...
    /**
     * Parses the XML response into the standard PMS array format
     */
    private function parseBookingResponse(string $xmlString): array
    {
        ...
        try {
            ...
            error_log("Agoda XML Parse Error: " . $e->getMessage());
        }
    ...
    private function sendRequest(string $payload, bool $returnResponse = false)
    {
        ...
        if ($httpCode !== 200) {
            error_log("Agoda API Error: HTTP $httpCode - $response");
